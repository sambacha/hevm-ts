...
describe('Msize', () => {
  ...
  it('Test Msize', () => {
    const bytecode = '608060405259'
    const executor: EVMExecutor = createExecutor(disassembler, bytecode, cfgCreator, opcodeExecutor)
    executor.run(0)
    expect(executor.evm.stack.get(0)).toEqual(Word.createLiteral('60'))
    expect(executor.evm.stack.length()).toEqual(1)
  })

  it('Test Msize 2', () => {
    const bytecode = '608060405260ff60605359'
    const executor: EVMExecutor = createExecutor(disassembler, bytecode, cfgCreator, opcodeExecutor)
    executor.run(0)
    expect(executor.evm.stack.get(0)).toEqual(Word.createLiteral('80'))
    expect(executor.evm.stack.length()).toEqual(1)
  })

  it('Test Msize 3', () => {
    const bytecode = '608060405260ff60605360ee60ff5259'
    const executor: EVMExecutor = createExecutor(disassembler, bytecode, cfgCreator, opcodeExecutor)
    executor.run(0)
    expect(executor.evm.stack.get(0)).toEqual(Word.createLiteral('120'))
    expect(executor.evm.stack.length()).toEqual(1)
  })
})
