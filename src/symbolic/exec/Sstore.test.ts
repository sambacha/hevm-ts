...
describe("Sstore", () => {
  ...
  it("Test Sstore", () => {
    ...
    expect(executor.evm.storage.load(Word.createLiteral("01"))).toEqual(
      Word.createLiteral("20"),
    );
  ...
  it("Test Sstore Symbol slot", () => {
    ...
    expect(
      executor.evm.storage.load(Word.createSymbolic(Symbols.CALLVALUE)),
    ).toEqual(Word.createLiteral("20"));
    expect(executor.evm.stack.length()).toEqual(0);
  });
  ...
  it("Test Sstore Symbol value", () => {
    ...
    expect(executor.evm.storage.load(Word.createLiteral("20"))).toEqual(
      Word.createSymbolic(Symbols.CALLVALUE),
    );
    expect(executor.evm.stack.length()).toEqual(0);
  });
});
