import { createExecutor, createEVMDisassembler } from "./TestUtils";
import { EVMExecutor } from "../EVMExecutor";
import { EthereumCFGCreator } from "@/../cfg/EthereumCFGCreator";
import { Disassembler } from "@/../bytecode/Disassembler";
import { OpcodeExecutor } from "./OpcodeExecutor";
import { EVMDisassembler } from "@/../bytecode/EVMDisassembler";
import { Word } from "../Word";
import { Symbols } from "../Symbols";

describe("Sar", () => {
  let cfgCreator: EthereumCFGCreator;
  let disassembler: Disassembler;
  let opcodeExecutor: OpcodeExecutor = new OpcodeExecutor();

  beforeEach(() => {
    cfgCreator = new EthereumCFGCreator();
    disassembler = createEVMDisassembler();
  });

  it("Test Sar 1", () => {
    const bytecode = "600160001d";
    const executor: EVMExecutor = createExecutor(
      disassembler,
      bytecode,
      cfgCreator,
      opcodeExecutor,
    );
    executor.run(0);
    expect(executor.evm.stack.get(0)).toEqual(Word.createLiteral("01"));
    expect(executor.evm.stack.length()).toEqual(1);
  });

  it("Test Sar 2", () => {
    const bytecode = "600160011d";
    const executor: EVMExecutor = createExecutor(
      disassembler,
      bytecode,
      cfgCreator,
      opcodeExecutor,
    );
    executor.run(0);
    expect(executor.evm.stack.get(0)).toEqual(Word.createLiteral("00"));
    expect(executor.evm.stack.length()).toEqual(1);
  });

  it("Test Sar 3", () => {
    const bytecode = "600260011d";
    const executor: EVMExecutor = createExecutor(
      disassembler,
      bytecode,
      cfgCreator,
      opcodeExecutor,
    );
    executor.run(0);
    expect(executor.evm.stack.get(0)).toEqual(Word.createLiteral("01"));
    expect(executor.evm.stack.length()).toEqual(1);
  });

  it("Test Sar 4", () => {
    const bytecode = "600460011d";
    const executor: EVMExecutor = createExecutor(
      disassembler,
      bytecode,
      cfgCreator,
      opcodeExecutor,
    );
    executor.run(0);
    expect(executor.evm.stack.get(0)).toEqual(Word.createLiteral("02"));
    expect(executor.evm.stack.length()).toEqual(1);
  });

  it("Test Sar 4", () => {
    const bytecode = "600f60011d";
    const executor: EVMExecutor = createExecutor(
      disassembler,
      bytecode,
      cfgCreator,
      opcodeExecutor,
    );
    executor.run(0);
    expect(executor.evm.stack.get(0)).toEqual(Word.createLiteral("07"));
    expect(executor.evm.stack.length()).toEqual(1);
  });

  it("Test Sar Symbolic", () => {
    const bytecode = "6020341d";
    const executor: EVMExecutor = createExecutor(
      disassembler,
      bytecode,
      cfgCreator,
      opcodeExecutor,
    );
    executor.run(0);
    expect(executor.evm.stack.get(0)).toEqual(
      Word.createSymbolic(Symbols.UNKNOWN),
    );
    expect(executor.evm.stack.length()).toEqual(1);
  });
});
