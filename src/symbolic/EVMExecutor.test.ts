...
describe("EVMExecutor", () => {
  ...
  it("Test block with JUMPI", () => {
    const bytecode =
      "6080604052348015600f57600080fd5b5060868061001e6000396000f3";
    const executor: EVMExecutor = createExecutor(
      disassembler,
      bytecode,
      cfgCreator,
  ...
  it("Test real contract binary - constructor", () => {
    ...
    const contract: DisassembledContract =
      disassembler.disassembleContract(bytecode);
    const constructor = contract.constructor;
    const executor = new EVMExecutor(
      cfgCreator.divideBlocks(constructor),
      opcodeExecutor,
    );
    executor.run(0);
