...
export class EVMDisassembler implements Disassembler {
  ...
  disassembleSourceCode(
    ...
    const contract = this.contractService.compileContract(
      contractName,
      source,
      path,
    );
    ...
    const asmRuntime = contractAssembly[".data"][0][".code"].filter(
      (elem) => elem.name !== "tag",
    );
    const asmConstructor = contract.evm.legacyAssembly[".code"].filter(
      (elem) => elem.name !== "tag",
    );
    const disassembledCode: DisassembledContract =
      this.disassembleContract(bytecode);
  ...
  disassembleBytecode(bytecode: string): Operation[] {
    ...
    for (let i = 0; i < operations.length; i++) {
      ...
      const opcode: Opcode =
        Opcodes.opcodes[parseInt(code, 16)] || Opcodes.opcodes[-1];
  ...
  private populateStartEnd(
    disassembledCode: DisassembledContract,
    asmRuntime,
    asmConstructor,
  ): DisassembledContract {
    ...
    if (
      constructor.length !== asmConstructor.length + 1 ||
      runtime.length !== asmRuntime.length + 1
    ) {
      logger.error(
        `Source mappings do not match with bytecode, constructorLength=${constructor.length}, asmConstructorLength=${asmConstructor.length}, runtimeLength=${runtime.length}, asmRuntimeLength=${asmRuntime.length}`,
      );
